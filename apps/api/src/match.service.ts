...
const jsonSafe = <T>(value: T): T => JSON.parse(JSON.stringify(value, (_, v) => typeof v === 'bigint' ? v.toString() : v));

@Injectable()
export class MatchService {
  constructor(private readonly prisma: PrismaService) {}
  async listOpen() { return jsonSafe(await this.prisma.match.findMany({ where: { state: MatchState.OPEN }, include: { participants: { include: { user: true } }, table: { include: { venue: true } } }, orderBy: { createdAt: 'desc' } })); }
  ...
  async create(dto: { challengerId: string; stakeMinor: number; tableId?: string }) {
    ...
    return jsonSafe(await this.prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { actorId: challenger.id, action: 'MATCH_CREATED', entity: 'Match', entityId: match.id, metadata: { demo: true } } }); return match;
    }));
  }
  ...
  async start(id: string) { return jsonSafe(await this.transition(id, MatchState.READY, MatchState.LIVE)); }
